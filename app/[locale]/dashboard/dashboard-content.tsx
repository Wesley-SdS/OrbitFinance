...
export async function DashboardContent({ userId, t }: { userId: string; t: any }) {
  const [recentTransactions, accounts, goals] = await Promise.all([
    getTransactionsCached(userId, { limit: 5 }),
    getAccountsCached(userId),
    getGoalsCached(userId),
  ])
